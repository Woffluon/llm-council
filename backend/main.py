...
async def global_exception_handler(request: Request, exc: Exception):
    """Global unhandled exception handler for all API requests."""
    error_trace = traceback.format_exc()
    logger.error(f"[Global Error] Unhandled Exception on {request.method} {request.url.path}:\n{error_trace}")
    return JSONResponse(
        status_code=500,
        content={
...
async def http_exception_handler(request: Request, exc: HTTPException):
    """HTTP exception handler for status code exceptions."""
...
async def get_models():
    """Get preset model configurations for providers."""
    from .config import (
    ...
    return {
        ...
        "nvidia_nim": {
            "council_models": NVIDIA_NIM_COUNCIL_MODELS,
            "chairman_model": NVIDIA_NIM_CHAIRMAN_MODEL,
        }
    }
...
async def send_message(conversation_id: str, request: SendMessageRequest):
    ...
    stage1_results, stage2_results, stage3_result, metadata = await run_full_council(
        ...
        custom_council_models=request.council_models,
        custom_chairman_model=request.chairman_model
    )
